...
export const getDashboard = async (req, res) => {
  try {
    const [authStats, shopalizeStats, watchtowerStats] = await Promise.allSettled([
      callService('auth', '/api/auth/internal/stats'),
      shopalizeService.getStoreStats(),
      watchtowerService.getPlatformOverview(),
    ]);

    // Get ripplify platform stats
    let ripplifyStats = { totalEarned: 0, totalPending: 0, totalTransactions: 0, totalSuccess: 0 };
    try {
      const ripplifyData = await ripplifyService.getTransactionStats();  // no userId = get all
      ripplifyStats = {
        totalEarned: ripplifyData.totalSuccess || 0,
        totalPending: ripplifyData.totalPending || 0,
        totalTransactions: ripplifyData.totalTransactions || 0,
      };
    } catch (err) {
      console.error('Ripplify stats error:', err);
    }
    ...
    res.json({
      ...
      totalTransactions: ripplifyStats.totalTransactions || 0,
      ripplifyRevenue: ripplifyStats.totalEarned || 0,
      ripplifyPending: ripplifyStats.totalPending || 0,
