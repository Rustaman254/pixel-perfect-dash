...
const FormsFeatureFlags = () => {
  ...
  const loadFlags = async () => {
    try {
      setLoading(true);
      const data = await fetchWithAuth('/admin/feature-flags');
      setFlags(data.filter((f: FeatureFlag) => f.key.startsWith('forms_') || f.category === 'ai'));
    } catch (error: any) {
  ...
  const handleToggle = async (flag: FeatureFlag) => {
    try {
      await fetchWithAuth(`/admin/feature-flags/${flag.id}/toggle`, {
        method: 'PATCH',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetchWithAuth(`/admin/feature-flags/${id}`, { method: 'DELETE' });
  ...
  const handleCreate = async () => {
    ...
    try {
      setCreating(true);
      const created = await fetchWithAuth('/admin/feature-flags', {
        method: 'POST',
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Forms Feature Flags</h1>
          <p className="text-sm text-slate-500">Control which features are available for the Forms application.</p>
        </div>
        <button onClick={() => setCreateOpen(true)} className="bg-orange-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-700 transition-colors flex items-center gap-2">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search features..." className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400 transition-all" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan={5} className="py-20 text-center"><Loader2 className="w-8 h-8 animate-spin text-orange-600 mx-auto mb-2" /><p className="text-sm text-slate-500">Loading features...</p></td></tr>
        ...
        <DialogContent className="max-w-md bg-white">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input value={newFlag.key} onChange={e => setNewFlag({ ...newFlag, key: e.target.value.toLowerCase().replace(/\s+/g, '_') })} placeholder="e.g. forms_new_feature" />
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <button onClick={handleCreate} disabled={creating} className="bg-orange-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-orange-700 disabled:opacity-50">
...
export default FormsFeatureFlags;