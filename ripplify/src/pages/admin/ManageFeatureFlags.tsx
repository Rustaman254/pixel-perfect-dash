...
import { Flag, Loader2, Plus, Trash2, ToggleLeft, ToggleRight, Search, Filter } from "lucide-react";
...
const ManageFeatureFlags = () => {
    ...
    const handleToggle = async (id: number, currentState: boolean) => {
        try {
            await fetchWithAuth(`/admin/feature-flags/${id}/toggle`, {
                method: 'PATCH',
                body: JSON.stringify({ isEnabled: !currentState })
            });
            setFlags(flags.map(f => f.id === id ? { ...f, isEnabled: !currentState } : f));
            toast({
                title: "Feature toggled",
                description: `Feature has been ${!currentState ? 'enabled' : 'disabled'}.`
            });
        } catch (error: any) {
            toast({ title: "Error", description: error.message, variant: "destructive" });
        }
        ...
        <AdminLayout>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Control which features are available to sellers on the platform.</p>
                </div>
                <button
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50/50">
                                ...
                                <th className="text-left px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-slate-400">Last Updated</th>
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan={6} className="py-20 text-center">
                                        ...
                                        <p className="text-sm text-slate-500 font-medium">Loading feature flags...</p>
                                    </td>
                                </tr>
                            ) : filteredFlags.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="py-20 text-center">
                                        <p className="text-sm text-slate-500 font-medium">No features found.</p>
                                    </td>
                                </tr>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={cn(
                                                ...
                                                flag.isEnabled ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"
                                            )}>
